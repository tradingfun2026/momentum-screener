...
DEFAULT_MIN_VOLUME    = 0            # 🔥 now 0 instead of 100_000
...
with st.sidebar:
    ...
    # 🔥 Min Daily Volume can now go down to 0 (UI change you requested)
...
def scan_one(sym, enable_enrichment: bool, enable_ofb_filter: bool, min_ofb: float):
    try:
        ...
        # 🔥 WATCHLIST PRECEDENCE: bypass price/volume gate if this is from watchlist
        if exchange != "WATCH":
